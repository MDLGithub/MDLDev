...
<?php
//saving mark_as_test data to both tblqualify and tbl_ss_qualify tables
if(isset($_POST['mark_as_test'])){
    $markedT1Users =$_POST['markedRow']['tblqualify']['user'];
    $markedT2Users =$_POST['markedRow']['tbl_ss_qualify']['user'];
    if($markedT1Users){
	foreach ($markedT1Users as $userID=>$v){
	    updateTable($db,'tblqualify', array('mark_as_test'=>'1'), array('Guid_user'=>$userID));
	}
    }
    if($markedT2Users){
	foreach ($markedT2Users as $userID=>$v){
...
$sqlIncomplates  = "SELECT q.*,p.*, u.email, q.Date_created AS `date`, 'Incomplete' as qualified FROM tblqualify q
		    LEFT JOIN tblpatient p ON q.Guid_user = p.Guid_user
		    LEFT JOIN tbluser u ON p.Guid_user = u.Guid_user
		    WHERE NOT EXISTS(SELECT * FROM tbl_ss_qualify qs WHERE q.Guid_qualify=qs.Guid_qualify)";
$whereIncomplate = " AND q.mark_as_test='0'";
...
$incomplate_requests .= $whereIncomplate;
$incomplate_requests  = $db->query($sqlIncomplates);
...
if(!isset($_POST['search'])){
    $qualify_requests = array_merge($qualify_requests, $incomplate_requests);
    $num_estimates = $qualify_requests;
    function cmp($a, $b){
	return strtotime($b['date']) - strtotime($a['date']);
    }
    usort($qualify_requests, "cmp");
}
?>
		...
		<div class="formContent">
		   ...
		   <tbody>
			...
			foreach ($qualify_requests as $qualify_request) {
			    ...
			    $tableName ="tbl_ss_qualify";
			    if($qualify_request['qualified']=="Incomplete"){
				$tableName ="tblqualify";
			    }
		    ?>
			    <tr data-id="<?php //echo $estimate_request['Guid_brcaestimate']; ?>" class="t_row">
				    <td class="printSelectBlock text-center">
					<input name="markedRow[<?php echo $tableName; ?>][user][<?php echo $qualify_request['Guid_user']; ?>]" type="checkbox" class="print1 report1" data-selected_questionnaire="<?php echo $qualify_request['Guid_qualify']; ?>" data-selected_date="<?php echo $qualify_request['date']; ?>" />
				    </td>
