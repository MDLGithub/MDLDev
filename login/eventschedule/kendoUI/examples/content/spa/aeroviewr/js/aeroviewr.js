var model,
    ...
    popularQuery = { url: "/photos", settings: { feature: "editors", exclude: "Nude", exclude_nude: 1, page: 1, image_size: 1, rpp: 50 } };
...
model = kendo.observable({
    ...
    searchFor: function(query, showFirst) {
        ...
        if (currentQuery !== query) {
            ...
            if (query) {
                photos.read({ url: '/photos/search', settings: { exclude: "Nude", exclude_nude: 1, term: query, page: 1, image_size: 1, rpp: 50 } });
            } else {
...
router.route("search/:query/photos/:id", function(query, id) {
    model.show(id);
    model.searchFor(decodeURIComponent(query), false);
});

router.route("search/:query", function(query) {
    model.searchFor(decodeURIComponent(query), true);
});
