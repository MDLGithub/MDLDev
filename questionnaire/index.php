...
function display_qualification($qualification_text, $not_qualified) {
	...
	if (($not_qualified) && ($insurance == "NCCN")) {
		$relatives = $conn->query("SELECT * FROM  tblqualifyans WHERE Guid_qualify=" . $_SESSION['id'] . " AND cancer_type = 'Breast' AND relative IN (SELECT value FROM (SELECT * FROM tblfirstdegrel UNION ALL SELECT * FROM tblseconddegrel) raw) AND Guid_question IN (SELECT Guid_question FROm tblcancerquestion WHERE question_type = 'family') AND age_relative <= 45");
		
		if (mysqli_num_rows($relatives)) {
			$not_qualified = 0;
			foreach($relatives as $relative) {
				$sql = "UPDATE tblqualifyans SET Guid_question = 2 WHERE Guid_qualifyans = " . $relative['Guid_qualifyans'];
...
function save_input() {
	...
	if (count($_POST['personal_cancer'])) {
		...
		if (in_array("No Cancer/None of the Above", $_POST['personal_cancer'])) {
			$conn->query("DELETE FROM tblunknownans WHERE Guid_qualify=" . $_SESSION['id'] . " AND Guid_question IN (SELECT Guid_question FROm tblcancerquestion WHERE question_type = 'personal')");			
		}
	}
	...
	if (count($_POST['family_cancer'])) {
		...
		if (in_array("No Cancer/None of the Above", $_POST['family_cancer'])) {
			$conn->query("DELETE FROM tblunknownans WHERE Guid_qualify=" . $_SESSION['id'] . " AND Guid_question IN (SELECT Guid_question FROm tblcancerquestion WHERE question_type = 'family')");			
		}
	}
...
?>
			<table class="tbl_summary">
				<thead>
					<tr>
						<th>Relative Cancer Not Provided</th>						
					</tr>
				</thead>
				<tbody>
				</tbody>
